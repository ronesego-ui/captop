...
class UTMDataUI(tk.Toplevel):
    def __init__(self, parent_app, company_id, company_name, period, model):
        ...
        self.title(f"Datos UTM - {company_name} (Período {period})")
    ...
    def _create_widgets(self):
        ...
        ttk.Label(header_frame, text="Datos Mensuales de la UTM", 
        ...
        # Área de datos
        data_frame = ttk.LabelFrame(main_frame, text="Datos UTM", padding="10")
        ...
        self.tree.heading("valor", text="Valor UTM")
    ...
    def _fetch_bcch_data(self):
        try:
            ...
            # Consultar datos al BCCh (serie para UTM)
            siete = bcch.Siete("ronesego@outlook.com", "24052Mil6")
            utm_mensual = siete.cuadro(
                series=["F073.UTR.PRE.Z.M"],  # Serie para UTM
                nombres=["UTM"],
                desde="2025-01-01",
            ...
            # Procesar datos
            df = utm_mensual.reset_index()
            df.columns = ['Fecha', 'Valor_UTM']
            df['Fecha'] = pd.to_datetime(df['Fecha']).dt.strftime('%Y-%m-%d')
            df['Valor_UTM'] = df['Valor_UTM'].astype(float)
    ...
    def _update_treeview(self, df):
        ...
        for _, row in df.iterrows():
            self.tree.insert("", "end", values=(
                row['Fecha'],
                f"{row['Valor_UTM']:,.0f}".replace(',', '.')  # Formatear con separadores
            ))
    ...
    def _save_data(self):
        try:
            ...
            for fecha, valor in data_to_save:
                if self.model.save_utm_data(fecha, valor):
                    count += 1
            
            self.status_var.set(f"Datos guardados: {count} registros")
            messagebox.showinfo("Éxito", f"Se guardaron {count} registros de UTM en la base de datos")
    ...
    def _load_existing_data(self):
        try:
            utm_data = self.model.get_utm_data()
            if utm_data:
                # Limpiar treeview
                for item in self.tree.get_children():
                    self.tree.delete(item)
                ...
                for fecha, valor in utm_data:
                    self.tree.insert("", "end", values=(
                        fecha,
                        f"{valor:,.0f}".replace(',', '.')  # Formatear con separadores
                    ))
                self.status_var.set(f"Datos cargados: {len(utm_data)} registros")
            else:
                self.status_var.set("No hay datos UTM almacenados")
                
        except Exception as e:
            logger.error(f"Error cargando datos UTM: {str(e)}")
