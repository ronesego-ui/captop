# ventaspagadasperiodoprofessional.py
...
class VentasPagadasProfessionalModel:
    """Modelo para manejar los datos de Ventas Pagadas en el Período PROFESSIONAL."""
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    (company_id, period, "VENTAS_PAGADAS_PROFESSIONAL", json.dumps(data))
                )
                conn.commit()
                return True
        except Exception as e:
            logger.error(f"Error saving ventas pagadas PROFESSIONAL data: {str(e)}")
            return False
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    (company_id, period, "VENTAS_PAGADAS_PROFESSIONAL")
                )
                row = cursor.fetchone()
                return json.loads(row["data"]) if row else None
        except Exception as e:
            logger.error(f"Error loading ventas pagadas PROFESSIONAL data: {str(e)}")
            return None

class VentasPagadasProfessionalUI(tk.Toplevel):
    """Interfaz gráfica para Ventas Pagadas en el Período PROFESSIONAL."""
    ...
    def __init__(self, parent_app, company_id: int, company_name: str, period: int):
        ...
        self.model = VentasPagadasProfessionalModel(db_file)
        
        # Inicializar variables
        self.entry_vars = {}
        
        self.title("Ventas Pagadas Período PROFESSIONAL")
    ...
    def _create_header(self):
        ...
        ttk.Label(header_frame, text="VENTAS PAGADAS EN EL PERÍODO PROFESSIONAL", 
    ...
    def _create_table(self):
        ...
        # Encabezados de columnas (mismos países que HOME para PROFESSIONAL)
        ...
        # Filas de la tabla (mismas que HOME para PROFESSIONAL)
        rows = [
            "TDI",
            "CD",
            "CO",
    ...
    def calculate_values(self):
        ...
        messagebox.showinfo("Información", "Los cálculos para PROFESSIONAL se realizarán según la lógica de negocio específica")
        ...
        try:
            ...
            if self.model.save_ventas_data(self.company_id, self.period_int, data):
                messagebox.showinfo("Éxito", 
                                  f"Datos de Ventas Pagadas PROFESSIONAL guardados para el período {self.period_int}")
            else:
...
def abrir_ventas_pagadas_professional(parent_app, company_id: int, company_name: str, period: int):
    VentasPagadasProfessionalUI(parent_app, company_id, company_name, period)