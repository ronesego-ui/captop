...
TRANSLATIONS = {
    "es": {
        "window_title": "Consulta Balance Final de la Empresa",
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    (company_id, period, "FINAL_BALANCE")
                )
                row = cursor.fetchone()
...
# ------------------------- Vista -------------------------
class FinalBalanceConsultaUI(tk.Toplevel):
    """Interfaz gráfica para la Consulta del Balance Final."""
    
    def __init__(self, parent_app, company_id: int, company_name: str, period: int):
        """Inicializa la ventana de Consulta de Balance Final."""
...
def abrir_consulta_balance_final(parent_app, company_id: int, company_name: str, period: int):
    """Abre la interfaz de consulta de balance final."""
    FinalBalanceConsultaUI(parent_app, company_id, company_name, period)