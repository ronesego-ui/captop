...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('datos_fisicos_inventario_consulta.log'),
...
TRANSLATIONS = {
    "es": {
        "window_title": "Consulta Datos Físicos de Inventarios Total Continente",
...
# ------------------------- Vista -------------------------
class DatosFisicosInventarioConsultaUI(tk.Toplevel):
    """Interfaz gráfica para Consulta de Datos Físicos de Inventarios."""
    ...
    def _create_inventory_sections(self):
        ...
        self._create_display_row(frame, tr("kits_units"), "kits_units", 0)
        self._create_display_row(frame, tr("ppa_units"), "ppa_units", 1)

        # Modelo Desktop
        frame = ttk.LabelFrame(self.scrollable_frame, text=tr("desktop_model"), padding=(10, 5))
        frame.pack(fill="x", padx=10, pady=10)
        
        self._create_display_row(frame, tr("assembly_period"), "desktop_assembly", 0)
        self._create_display_row(frame, tr("finished_products"), "desktop_finished", 1)

        # Modelo Notebook
        frame = ttk.LabelFrame(self.scrollable_frame, text=tr("notebook_model"), padding=(10, 5))
        frame.pack(fill="x", padx=10, pady=10)
        
        self._create_display_row(frame, tr("notebook_assembly"), "notebook_assembly", 0)
        self._create_display_row(frame, tr("notebook_finished"), "notebook_finished", 1)
        
    def _create_display_row(self, parent_frame, label_text, key, row_num):
        ...
        # Usar un Label en lugar de Entry para mostrar el valor (no editable)
        display = ttk.Label(row_frame, textvariable=var, width=15, background="#F0F0F0", 
                           borderwidth=1, relief="solid", anchor="e", padding=2)
...
def abrir_consulta_estadoderesultado(parent_app, company_id: int, company_name: str, period: int):
    ventana = DatosFisicosInventarioConsultaUI(parent_app, company_id, company_name, period)
    ventana.grab_set()