...
TEXTS = {
    "window_title": "Precios de Materias Primas",
...
class RawMaterialPriceUI(tk.Toplevel):
    """Interfaz gráfica para la sección de Precios de Materias Primas."""
    ...
    def _create_widgets(self):
        ...
        ttk.Label(
            ...
            text="Precios de Materias Primas", 
            font=('Inter', 18, 'bold'), 
            anchor='center'
    ...
    def _create_home_notebook_section(self):
        ...
        for i, item in enumerate(items_home):
            ...
            ttk.Entry(frame, textvariable=self.entry_vars[f"{item}_60_days"]).grid(row=i+1, column=1, padx=5, pady=2, sticky='ew')
            self.entry_vars[f"{item}_120_days"] = tk.StringVar()
            ttk.Entry(frame, textvariable=self.entry_vars[f"{item}_120_days"]).grid(row=i+1, column=2, padx=5, pady=2, sticky='ew')
    ...
    def _create_other_rates_section(self):
        ...
        for i, item in enumerate(other_items):
            ...
            ttk.Entry(frame, textvariable=self.entry_vars[self._clean_key(item)]).grid(row=i, column=1, padx=5, pady=2, sticky='ew')
    ...
    def _save_data(self):
        ...
        raw_material_prices_data = {}
        for key, var in self.entry_vars.items():
            raw_material_prices_data[key] = self._get_numeric_value(var.get())
        
        # Cargar el payload existente para el período y actualizar solo las decisiones de materias primas
        existing_data = self.db.load_decision(self.company_id, self.period) or {}
        existing_data["raw_material_prices"] = raw_material_prices_data
