...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('balance.log'),
...
TRANSLATIONS = {
    "es": {
        "window_title": "Balance Inicial de la Empresa",
        ...
        "save_success": "Balance guardado para el período {period} de {company}.",
        ...
        "save_error": "Error al guardar balance: {error}",
...
class BalanceSheetModel:
    ...
    ''' Crea la tabla de balance inicial si no existe'''
    ...
    def save_balance_sheet(self, company_id: int, period: int, data: Dict[str, Any]) -> bool:
        """Guarda los datos del balance en la base de datos."""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "REPLACE INTO financial_statement (company_id, period, type, data) VALUES (?, ?, ?, ?)",
                    (company_id, period, "BALANCE_SHEET", json.dumps(data))
                )
                conn.commit()
                return True
        except Exception as e:
            logger.error(f"Error saving balance sheet: {str(e)}")
            return False
...
# ------------------------- Vista -------------------------
class BalanceSheetUI(tk.Toplevel):
    """Interfaz gráfica para el Balance Inicial."""
    
    def __init__(self, parent_app, company_id: int, company_name: str, period: int):
        """Inicializa la ventana de Balance Inicial."""
    ...
    def _configure_styles(self):
        ...
        style.configure('TCombobox', fieldbackground='white', borderwidth=1, relief='solid', padding=2)
    ...
    def _create_buttons(self):
        ...
        ttk.Button(button_frame, text=tr("save"), command=self.save_balance_sheet).grid(
    ...
    def _get_float_value(self, key: str) -> float:
        ...
        try:
            # Normalizar la clave solicitada
            ...
            # 2. Intentar variaciones comunes para claves específicas
            key_variations = {
                'activo_fijo_Acum_Periodos__Anteriores': [
                    'activo_fijo_Acum__Periodos_Anteriores',
                    'activo_fijo_acum_anteriores',
                    'activo_fijo_depreciacion_acumulada',
                    'activo_fijo_depreciacion_anteriores',
                    'acum_periodos_anteriores',
            ...
            # Buscar variaciones para la clave solicitada
            ...
            # 3. Búsqueda flexible por similitud
            # Primero en las variables de entrada
            for existing_key in self.entry_vars:
                # Buscar por inclusión en cualquier dirección
                if normalized_key in existing_key or existing_key in normalized_key:
                    value = self.entry_vars[existing_key].get()
                    return float(value) if value else 0.0
                    
            # Luego en las variables de totales
            for existing_key in self.total_vars:
                if normalized_key in existing_key or existing_key in normalized_key:
                    value = self.total_vars[existing_key].get()
                    return float(value.replace(',', '')) if value else 0.0
            
            # 4. Si todo falla, registrar advertencia y devolver 0
            logger.debug(f"Clave no encontrada: {normalized_key} (todas las búsquedas fallaron)")
            return 0.0
            ...
            logger.error(f"Error inesperado en _get_float_value: {str(e)}")
            return 0.0


#"""Carga los datos iniciales del balance desde JSON o base de datos."""
    def _load_initial_data(self):   
             
        json_filename = "balance_inicial.json"
        json_path = Path(__file__).parent.parent / "json" / json_filename
    
        logger.info(f"Intentando cargar datos iniciales desde: {json_path}")
        ...
        try:
            if json_path.exists():
                ...
                logger.info(f"Archivo {json_filename} encontrado, cargando datos...")
                
                # Asignar valores a las entradas
                for key, var in self.entry_vars.items():
                    if key in json_data:
                        var.set(str(json_data[key]))
                        logger.debug(f"Valor cargado para {key}: {json_data[key]}")
                    else:
                        logger.debug(f"Clave no encontrada en JSON: {key}")
                
                # Asignar valores a los totales
                for key, var in self.total_vars.items():
                    if key in json_data:
                        var.set(f"{json_data[key]:,.2f}")
                        logger.debug(f"Total cargado para {key}: {json_data[key]}")
                    else:
                        logger.debug(f"Clave de total no encontrada en JSON: {key}")
                
                # Guardar en base de datos para futuros accesos
                if self.model.save_balance_sheet(self.company_id, self.period_int, json_data):
                    logger.info("Datos iniciales guardados en base de datos")
                else:
                    logger.error("Error al guardar datos iniciales en base de datos")
                
                messagebox.showinfo("Carga Inicial", 
                                f"Datos de balance cargados desde {json_filename}")
                self.calculate_totals()
                return
            else:
                logger.warning(f"Archivo {json_filename} no encontrado en {json_path}")
                
        except Exception as e:
            logger.error(f"Error al cargar {json_filename}: {str(e)}")
            messagebox.showerror("Error", f"Error al cargar archivo inicial: {str(e)}")

        
        # Si no hay archivo JSON o hubo error, cargar desde base de datos
    ...
    def calculate_totals(self):
        """Calcula los totales del balance con validación robusta."""
        try:
            # --- Activo Circulante ---
            ...
            acum_depreciacion = self._get_float_value("activo_fijo_Acum__Periodos_Anteriores")
    ...
    def save_balance_sheet(self):
        ...
        try:
            ...
            if self.model.save_balance_sheet(self.company_id, self.period_int, balance_data):
            ...
            logger.error(f"Error inesperado al guardar balance: {str(e)}")
            messagebox.showerror("Error", 
